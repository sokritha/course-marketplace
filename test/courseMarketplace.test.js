...
const getGas = async (result) => {
  // Getting the transaction in order to get the gas fee
...
contract("CourseMarketplace", (accounts) => {
  const courseId = "0x00000000000000000000383936343634";
  const proof =
    "0x27501d6169233cf75694c1d395b9654c7db432d0da1d608ff2a8eca9dd9619b6";
  ...
  describe("Purchase the new course", () => {
    ...
    it("can get the purchased course hash by index", async () => {
      ...
      assert.equal(
        courseHash,
        expectedHash,
        "Course hash is not matching the one created with the hash of purchased course."
      );
    });

    it("should match the data of the course purchase by buyer", async () => {
  ...
  describe("Trasfer ownership", () => {
    ...
    it("getContractOwner should return deployer address", async () => {
      assert.equal(
        contractOwner,
        currentOwner,
        "Contract owner is not matching with the one from getContractOwner function"
      );
    });
  ...
  describe("Deactivate course", () => {
    ...
    it("should have status of deactivated and price 0", async () => {
      ...
      assert.equal(course.state, expectedState, "Coruse is NOT deactivated!");
      assert.equal(course.price, expectedPrice, "Coruse price is NOT 0!");

      assert.equal(
        toBN(beforeTxOwnerBalance).sub(gas).toString(),
        afterTxOwnerBalance,
        "Contarct owner balance is not correct"
      );
  ...
  describe("Normal withdraw", () => {
    ...
    const overLimitFunds = "999000000000000000000";
    let currentOwner = null;
